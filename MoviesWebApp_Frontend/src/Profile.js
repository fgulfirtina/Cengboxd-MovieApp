...
const Profile = () => {
    const { user, isLoggedIn, fetchUserProfile, logout } = useContext(AuthContext);
    ...
    const [preview, setPreview] = useState(user?.profilePicture || 'https://cdn.pixabay.com/photo/2015/10/05/22/37/blank-profile-picture-973460_1280.png');
    ...
    useEffect(() => {
        const checkUserState = async () => {
            if (!user) {
                ...
                // Update the preview if a profile picture URL exists
                if (user.profilepic) {
                    setPreview(`https://localhost:44398${user.profilepic}`);
                }
            }
        };
    ...
    const handleDeleteAccount = async () => {
        ...
        try {
            ...
            const response = await axios.delete(`https://localhost:44398/delete-account/${userId}`);
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            // Update profile picture preview if a new one is provided
            if (response.data.profilePictureUrl) {
                // Update preview with the full URL to the image
                setPreview(`https://localhost:44398${response.data.profilePictureUrl}`);
            }
